...
=head2 CNVnator pipeline launcher
 ...
 accept path to samtools, genomefile
...
# Defaults
$refFai   ||= $ENV{HOME}."/data/reference/hg19_random/fasta/UCSC/hg19_random.fa.fai";
$samtools ||= $ENV{HOME}."/bin/samtools";
$rootdir  = `which root`;
$rootdir  =~s!bin/root!!;
chomp($rootdir);
$cnvnator = `which cnvnator`;
chomp($cnvnator);
...
if (!$cnvnator || !-e $cnvnator) {die "cnvnator needs to be loaded with \"module load cnvnator\" to run this analysis\n";}
...
$ENV{ROOTSYS} = $rootdir;
# I doubt that the below line is really needed
