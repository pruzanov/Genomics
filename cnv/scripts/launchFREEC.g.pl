...
=head2 Running FREEC for germline analysis
 ...
 ./launchFREEC.pl --input-bam data.bam --freec /bin/freec --id someID --lenfile hg19.len --samtools /bin/samtools --r-libdir My/R_modules

=cut

my $USAGE = "launchFREEC.pl --input-bam [bam input] --data-type [optional, default is WG, EX supported]".
...
# Required parameters
my($input,$type,$id,$config,$lenfile,$samtools,$freec,$prefix,$chrfiles);
# Optional parameters
my($datadir,$ploidy,$makebedgraph,$matetype,$logfile,$targetFile,$cvar,$quiet,$window,$rlibdir);
my $results = GetOptions ("input-bam=s"    =>  \$input,
                          "r-libdir=s"     =>  \$rlibdir,
                          "samtools=s"     =>  \$samtools,
                          "lenfile=s"      =>  \$lenfile,
                          "freec=s"        =>  \$freec,
...
if (!$input || !$samtools || !$freec || !$id || !$rlibdir) { die $USAGE; }
...
window   = WINDOW_TAG
chrFiles = CHRFILES_TAG

[sample]

mateFile = EXPERIMENT_TAG
inputFormat = BAM
...
$configBody=~s/EXPERIMENT_TAG/$input/;
$configBody=~s/MATE_TAG/$matetype/g;
$configBody=~s/CHRFILES_TAG/$chrfiles/;
...
# RUNNING FREEC here
#=====================================
if (!-e $freec || !-e $input || !-e $lenfile) { die "Invalid parameters, make sure all files exist and run again!";}
...
=head2
 ...
 File will be named [$input]_CNVs.p.value.txt
