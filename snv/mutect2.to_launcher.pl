...
while (<INPUTFILE>) {
 ...
 my($inT, $outVcf) = split("\t");
 next if (!$inT || !$outVcf);
 if($seendirs{dirname($outVcf)}++) { die "Place Output files in separate directories to avoid collision between temporary data";}
